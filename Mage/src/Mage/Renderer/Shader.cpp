...
struct Shader::Impl {
  ...
  void compile(const char *vertex_code, const char *fragment_code) {
    ...
    if (shader_id == 0u) {
      throw Exception(
          ("Failed creating shader program: " + std::string(shader_name))
              .c_str());
    }
    ...
    if (result == 0) {
      ...
      throw Exception(
          ("Error linking shader program: " + std::string(error_log)).c_str());
    }
    ...
    if (result == 0) {
      glGetProgramInfoLog(shader_id, sizeof(error_log), nullptr, error_log);
      throw Exception(
          ("Error validating shader program: " + std::string(error_log))
              .c_str());
    }
  }
  ...
  void add(const char *shader_code, const GLenum shader_type) const {
    ...
    if (result == 0) {
      ...
      message << "Error compiling shader (shader type " << shader_type
              << "): " << error_log;
...
Shader::Shader(const char *shader_name, const char *vertex_shader_src,
...
uint32_t Shader::get_shader_id() const { return _impl->shader_id; }

void Shader::use() const { glUseProgram(_impl->shader_id); }
