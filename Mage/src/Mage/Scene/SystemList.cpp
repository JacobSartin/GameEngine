...
SystemList::Iterator::~Iterator() { delete _impl; }
...
SystemList::ConstIterator::~ConstIterator() { delete _impl; }

SystemList::ConstIterator::reference
...
bool operator==(const SystemList::ConstIterator &a,
                const SystemList::ConstIterator &b) {
  return a._impl->ptr == b._impl->ptr;
}
...
SystemList::~SystemList() { delete _impl; }

System *SystemList::at(size_t pos) { return _impl->list[pos]; }

const System *SystemList::at(size_t pos) const { return _impl->list[pos]; }

System *SystemList::operator[](size_t pos) { return _impl->list[pos]; }

const System *SystemList::operator[](size_t pos) const {
  return _impl->list[pos];
}

System *SystemList::front() { return _impl->list[0]; }

const System *SystemList::front() const { return _impl->list[0]; }

System *SystemList::back() { return _impl->list[_impl->count - 1]; }

const System *SystemList::back() const { return _impl->list[_impl->count - 1]; }

System **SystemList::data() { return _impl->list; }

const System **SystemList::data() const {
  return const_cast<const System **>(_impl->list);
}

bool SystemList::empty() const { return _impl->count <= 0; }

size_t SystemList::size() const { return _impl->count; }

SystemList::Iterator SystemList::begin() { return Iterator(&_impl->list[0]); }
