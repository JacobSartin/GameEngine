...
struct Log::Impl {
  void create_engine_logger() {
    ...
    if (engine_logger == nullptr) {
      auto sinks = create_sinks(engine_console_sink_enabled,
                                engine_file_sink_enabled, engine_log_file,
                                engine_log_file_size, engine_log_files_to_keep);
      engine_logger =
          std::make_shared<spdlog::logger>("E", sinks.begin(), sinks.end());
  ...
  void create_application_logger() {
    ...
    if (application_logger == nullptr) {
      auto sinks = create_sinks(application_console_sink_enabled,
                                application_file_sink_enabled,
                                application_log_file, application_log_file_size,
                                application_log_files_to_keep);
      application_logger =
          std::make_shared<spdlog::logger>("A", sinks.begin(), sinks.end());
  ...
  create_sinks(bool console_sink_enabled, bool file_sink_enabled,
               const std::string &log_file, uint32_t log_file_size,
               uint32_t log_files_to_keep) {
  ...
  static void
  apply_logger_format(const std::shared_ptr<spdlog::logger> &logger) {
  ...
  std::shared_ptr<spdlog::sinks::stdout_color_sink_mt> engine_console_sink =
      nullptr;
  std::shared_ptr<spdlog::sinks::rotating_file_sink_mt> engine_file_sink =
      nullptr;
  std::shared_ptr<spdlog::sinks::stdout_color_sink_mt>
      application_console_sink = nullptr;
  std::shared_ptr<spdlog::sinks::rotating_file_sink_mt> application_file_sink =
      nullptr;
  std::shared_ptr<spdlog::logger> engine_logger = nullptr;
  std::shared_ptr<spdlog::logger> application_logger = nullptr;
};

Log::Log() { _impl = new Impl(); }
...
void Log::set_engine_file_sink_parameters(const char *log_file,
...
void Log::set_application_file_sink_parameters(const char *log_file,
                                               uint32_t log_file_size,
                                               uint32_t log_files_to_keep) {
  std::lock_guard lock(_impl->sync_object);
