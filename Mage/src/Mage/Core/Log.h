...
// Define convenient logging macros for engine and application loggers for debug
// and info. But in release build, elid debug and info logging.
#ifdef NDEBUG
#define LOG_E_DEBUG(...)
#define LOG_E_INFO(...)
#define LOG_DEBUG(...)
...
public:
  ...
  void set_engine_file_sink_parameters(const char *log_file,
  ...
  void set_application_file_sink_parameters(const char *log_file,
                                            uint32_t log_file_size,
                                            uint32_t log_files_to_keep);
